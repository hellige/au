...
bool isRePattern(std::string_view sv, const TCLAP::SwitchArg &noRegexFlag) {
  return !noRegexFlag.isSet() && sv.starts_with("/") && sv.ends_with("/");
}

std::unique_ptr<re2::RE2> tryMakeRe(std::string_view sv) {
  const std::string_view actualPattern{sv.begin() + 1, sv.end() - 1};
...
void usage(const char *cmd) {
  ...
  std::cout
      ...
      << "    string in the form \"/.../\", where ... can be any regular expression. For\n"
      << "    example, the following could be used to ignore case while matching a value:\n"
      << "\n"
      << "      au " << cmd << " \"/(?i)somevalue/\" <path>...\n"
      << "\n"
